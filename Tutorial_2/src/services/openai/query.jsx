import { RetrieveRooms } from "../firebase/rooms/RetrieveRooms";
...
export async function query({ apiKey, dialogues, input, allRooms, setLoading, setResponse }) {
    setLoading(true);
    try {
...
All Rooms in ID KAIST: 
 ...
 Based on the previous dialogue and the user's current input, you need to detect the user's intentType and provide the necessary information to help the user.
 Possible types of intentType are "greeting", "reserve a room", "list all rooms", "view reservations", "cancel a reservation","get help", and "others".  
...
`},
                    ...
                    json_schema: {
                        ...
                        schema: {
                            ...
                            properties: {
                                intentType: {
                                    type: "string",
                                    enum: ["greeting", "reserve a room", "list all rooms", "check room availability", "cancel a reservation", "get help"]
                                },
                                responseForGreeting: {
                                    type: "object",
                                ...
                                paramsForReservation: {
                                    type: "object",
                                    description: "Parameters for reservation",
                                    properties: {
                                        startDateTime: { type: "string", format: "datetime" },
                                        endDateTime: { type: "string", format: "datetime" },
                                        room: {
                                            type: "string",
                                ...
                                paramsForCancelReservation: {
                                    type: "object",
                                    description: "Parameters for cancel reservation",
                                    properties: {
        ...
        if (parsedJSON.intentType === "greeting") {
            ...
            // setResponse(`Reservation ID: ${parsedJSON.paramsForReservation.reservation_id}`);
            const reservationForm = <>
                <span>공간을 새로 예약하고 싶어하시는군요. 아래 표에서 누락된 정보를 알려주신 다음, [확인] 버튼을 눌러주세요.</span>
                <table className="ReservationForm"><tbody>
            ...
            if (reservations.length==0) {
                setResponse(`해당 기간에 예약된 내역이 없습니다.`);
            } else {
        ...
        console.error(err);
        setResponse('Error occurred. Can you try again? 가끔 에러가 발생할 수 있습니다. 다시 한번 시도해주세요.');
    }
    setLoading(false);
